(() => {
    function setup() {
        ...
        return {
            ...
            numberOfAnowBalls: 250
        }
    }
    function random(min, max) {
        return Math.floor(Math.random() * (max - min + 1)) + min;
    }
    function createSnowBalls(canvas, numberOfAnowBalls) {
        return [...Array(numberOfAnowBalls)].map(() => {
            return {
    ...
    function run() {
        const { canvas, canvasContext, numberOfAnowBalls } = setup();
        const snowBalls = createSnowBalls(canvas, numberOfAnowBalls);
        setInterval(() => {
